...
//				printf("%2d <- %08x   @ %.0lf   -  %08x \n", target, regs[target].to_int(), sc_time_stamp().to_double() / 1000.0, backInst.address.to_int());
		...
		if (PRINT) {
            cout << ";" << "WRITEBACK;";
            cout << "0x" << std::hex << std::setw(8) << std::setfill('0') << static_cast<uint32_t>(backInst.I);
        }
	}
...
//	puede ayudar a depurar	cout << INST << endl;
	...
	if (PRINT) {
        cout << endl
             << ";DECODE;" << "0x" << std::hex << std::setw(8) << std::setfill('0') << static_cast<uint32_t>(INST.I) << ";" << INST.desc;
	}
