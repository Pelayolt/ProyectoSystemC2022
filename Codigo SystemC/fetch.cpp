...
void fetch::registro() {
    ...
    if (rst.read()) {
    ...
    } else if (miss_pending) {
        fillLine(miss_address);
        miss_pending = false;
        INST.I = 0x00000013;
        INST.address = miss_address;
        PCout.write(miss_address);
        cout << "Cargada nueva fila en la cache:" << endl;
        ...
        if (hit) {
            ...
            PCout.write(PC);
            cache_hits++;
            cout << "Instruccion " << hex << INST.I << " encontrada en la cache:" << endl;
...
bool fetch::fillLine(sc_uint<32> addr) {
    // Índice y tag de L1
    sc_uint<32> index = (addr >> 2) / WORDSPERLINE_L1 % NUMLINES_L1;
    sc_uint<32> tag = addr >> (2 + int(log2(WORDSPERLINE_L1)) + int(log2(NUMLINES_L1)));

    // Dirección base alineada a línea L2
    sc_uint<32> baseAddrL2 = addr & ~(WORDSPERLINE_L2 * 4 - 1);

    std::vector<sc_uint<32>> lineaL2;
    bool l2_hit = instCacheL2->read(baseAddrL2, true, lineaL2);// Lee línea completa L2 (palabras)

    // Calcular offset de la línea L1 dentro de línea L2
    unsigned offset = ((addr >> 2) & (WORDSPERLINE_L2 - 1)) / WORDSPERLINE_L1;

    // Extraer bloque L1 de la línea L2
    std::vector<sc_uint<32>> lineaL1(WORDSPERLINE_L1);
    for (unsigned i = 0; i < WORDSPERLINE_L1; i++) {
        lineaL1[i] = lineaL2[offset * WORDSPERLINE_L1 + i];
    }

    // Construir línea L1 para almacenar
    cacheLine newline;
    newline.valid = true;
    newline.tag = tag;
    newline.data = lineaL1;
    ...
    // Política de reemplazo L1
    if (set.ways.size() >= ASSOCIATIVITY_L1) {
        if (USEFIFO_L1) {
            set.ways.pop_front();
        } else {
            auto lru_it = std::min_element(set.ways.begin(), set.ways.end(),
